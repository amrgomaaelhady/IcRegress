...
############ This file is mainly to create environment and to visulaise.######

### when you run the enviornment alone it generates ranom action and plots the ground truth, but this feature is not perfect  #####
...
class ArmEnv(object):
    action_bound = [-1, 1]     ### action boundary as we are using tanh activation
    action_dim = 2
    state_dim = 3
    dt = .01  # refresh rate
    viewer = None
...
############# most of the variables are not used, just initialised. #######
############  here we map the values from -50 to 50 , to -1 to 1 #####################
    def __init__(self, mode='easy'):
        self.mode = mode
        self.center_coord = np.array(self.viewer_xy) / 2
        self.df1 = pd.read_csv("x_part_data_train1564_val174_test543.csv")
...
##### this function gives us the next step (that is current step ) ###########
    ...
    def step(self, action, index):
        ...
        ##### the rewrd function should be assigned here, here we are using MRdE error function   ##############
...
##### just to reset the values at the beiging  #################################################
...
##  here the state dimension is three : modality diffrence, pointing and gazing ######
    ...
    def _get_state(self,ind):
        ...
        self.p_a = self.df1["normalized_p_a"][ind]
        self.g_a = self.df1["normalized_g_a"][ind]
        in_point = 1 if self.grab_counter > 0 else 0    ##### this counter is not used 
        return np.hstack([self.modality_difference,self.p_a, self.g_a]), ind



##### these are for visualisation, no functionality. Can be ignored.  ######################################
###### It drwas three boxes on a car map two show the angles just for trial #####################
