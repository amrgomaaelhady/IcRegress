...
#    env.set_fps(30)
    ...
    while True:
            ...
            ind =ind + 1
            if(ind> TEST_IND):
                target = df1["target"][(TRAIN_IND+1):]
                tt = mean_squared_error(target, p_values)
                tt1 = abs(p_values - target) / df1['h_B_w'][(TRAIN_IND+1):]
...
if __name__ == '__main__':
    ...
    for i in range (20, 150, 2):
        for j in range(5500, 9500, 250):
            MAX_EP_STEPS = int(i)
            MEMORY_CAPACITY = int(j)
            M = Memory(MEMORY_CAPACITY, dims=2 * STATE_DIM + ACTION_DIM + 1)
            print(i, j)
            LOAD = False
            f = r"C:\Users\Kiran\Downloads\ML-PersRef-main\x_Final_code\Dataset 1 Reward function 3\Base_model"
