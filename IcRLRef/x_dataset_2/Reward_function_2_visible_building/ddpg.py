...
#    env.set_fps(30)
    ...
    while True:
            ...
            if(ind> TEST_IND):
                ...
                print('accuracy G1, G2, G3, G4', 100- (ac1 * 100 / oc1) , 100 - (ac2 * 100 / oc2) , 100- (ac3 * 100 / oc3) , 100- (ac4 * 100 / oc4))
                print('mrde G1, G2, G3, G4', mrde1/ oc1, mrde2/oc2, mrde3/oc3, mrde4/oc4)
                print('rmse G1, G2, G3, G4', np.sqrt(rmse1/oc1), np.sqrt(rmse2/oc2),np.sqrt(rmse3/oc3), np.sqrt(rmse4/oc4))
...
#    env.set_fps(30)
    ...
    while True:
            ...
            if(ind> V_I):
                ...
                target = df1["gtv_average"][(T_I+1):V_I+1]

                tt = mean_squared_error(target, p_values)

                hbw = abs(df1['gtv_min_average'][T_I+1: V_I+1] - df1['gtv_max_average'][T_I+1: V_I+1]) / 2
                tt1 = abs(p_values - target) / hbw
                ...
                rmse1 = rmse1 * (180 / math.pi)
                tt = tt * (180 / math.pi)
                print('Validation avg acc rmse mrde', 100- ((ac1 * 100 / oc1 )), np.sqrt(tt), tt2/10)
                with open('results.txt', 'a') as file:
                    # Write text to the file
                    file.write(f"{q} {w} {np.sqrt(tt)} {tt2}\n")
                #df8['asd'] = p_values[:306]
...
#    env.set_fps(30)
    ...
    while True:
            ...
            ind =ind + 1
            if(ind> 2294):
                target = df1["gtv_average"]
                df2 = df1
                #print(p_values[0], target[1622], df2["target"][0])
                hbw = abs(df1['gtv_min_average'] - df1['gtv_max_average']) / 2
                tt1 = abs(p_values - target) / hbw
                ...
                df2.to_csv('predicted_data_sec_train.csv', index=False)
                #print(sum(tt))
                #print(len(p_values))
