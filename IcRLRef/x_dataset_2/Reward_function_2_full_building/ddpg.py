...
TRAIN_IND =2294   #total length of the train -1
TEST_IND = 2951   # total length (train + test) -1
...
BATCH_SIZE = 64
FILE = "shuffled_sec_part_data.csv"
...
class Actor(object):
    ...
    def learn(self, s):  # batch update
        policy_grads, _ = self.sess.run([self.policy_grads, self.train_op], feed_dict={S: s})
        if self.t_replace_counter % self.t_replace_iter == 0:
            self.sess.run(self.replace)
        self.t_replace_counter += 1
        # Calculate the magnitude of the policy gradients as a proxy for loss
        loss = np.mean([np.linalg.norm(grad) for grad in policy_grads])
...
def train():
       ...
       # print(s)
        ...
        for t in range(MAX_EP_STEPS):
            ...
            if M.pointer > MEMORY_CAPACITY:
                ...
                b_s_ = b_M[:, -STATE_DIM:]
                los = critic.learn(b_s, b_a, b_r, b_s_)
                loss = actor.learn(b_s)
            ...
            if t == MAX_EP_STEPS - 1 or done:
                ...
                with open('rewards_l2.txt', 'a') as file:
                    # Write text to the file
                    file.write(f"{ep1} {ep_reward} \n")
                break
    ...
    loss_data = pd.DataFrame({
        'Training Step': range(len(actor_losses)),
        'Actor Loss': actor_losses,
        'Critic Loss': critic_losses
    })
    csv_file_path = 'losses.csv'
    loss_data.to_csv(csv_file_path, index=False)
    plt.figure(figsize=(8, 6))
    plt.scatter(range(len(actor_losses)), actor_losses, label='Actor Loss')
    ...
    plt.figure(figsize=(8, 6))
    plt.scatter(range(len(critic_losses)), critic_losses, label='Actor Loss')
    plt.xlabel('Training Steps')
    plt.ylabel('Loss')
    plt.title('Actor Loss over Time')
    plt.legend()
...
#    env.set_fps(30)
    ...
    while True:
            ...
            a = actor.choose_action(s)
            prediction = a[0] * 1.5
            p_values.append(a[0] * 1.5)
            s_, r, done = env.step(a, ind)
            # print(prediction)
            # s = s_
            ind = ind + 1
            if (ind > TEST_IND):
                target = df1["gtl_average"][(TRAIN_IND + 1):]
                tt = mean_squared_error(target, p_values)
                hbw = abs(df1['gtl_min_average'][TRAIN_IND + 1:] - df1['gtl_max_average'][TRAIN_IND + 1:]) / 2
                tt1 = abs(p_values - target) / hbw
                tt2 = sum(tt1) / len(p_values)
                participants = ['P28', 'P74', 'P22', 'P26', 'P32', 'P52', 'P21', 'P66', 'P71', 'P19', 'P58', 'P60']
...
#    env.set_fps(30)
    ...
    while True:
            ...
            if(ind> V_I):
                print(len(p_values))
                target = df1["gtl_average"][(T_I+1):V_I+1]
                print(len(target))
                tt = mean_squared_error(target, p_values)

                hbw = abs(df1['gtl_min_average'][T_I+1: V_I+1] - df1['gtl_max_average'][T_I+1: V_I+1]) / 2
                tt1 = abs(p_values - target) / hbw
                print(sum(tt1))
                tt2 = sum(tt1) / len(p_values)
                count = 0
                ...
                rmse1,rmse2,rmse3,rmse4 = 0,0,0,0
                selected_i_indices = []
                for i in range((T_I+1), V_I):
                        oc1 = oc1 + 1
                        mrde1 = mrde1 + tt1[i]
                        rmse1= rmse1 + ((p_values[i - (T_I + 1)] - target[i]) * (p_values[i - (T_I + 1)] - target[i]))
                       # rmse1= rmse1 + np.sqrt(tt[i])
                ...
                rmse1 = rmse1 * (180 / math.pi)
                tt = tt * (180 / math.pi)
                print('avg acc rmse mrde', 100- ((ac1 * 100 / oc1 )), np.sqrt(tt), tt2/10)
                print(q, w )
                with open('results.txt', 'a') as file:
                    # Write text to the file
                    file.write(f"{q} {w} {np.sqrt(tt)} {tt2}\n")
...
if __name__ == '__main__':
    ...
    for i in range (22, 150, 20):
        for j in range(5500, 15000, 1000):
            MAX_EP_STEPS = int(i)
